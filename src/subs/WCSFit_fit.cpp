...
//void FitClass::setupMaps() {
...

    {
      ...
      for (auto const & extnptr : extensions) {
        ...
        cerr << "check 4.33" << endl;
        cerr << extnptr->mapName << endl;
        cerr << "check 4.331" << endl;
        cerr << to_string(pmcInit->getFixed(extnptr->mapName)) << endl;
        cerr << "check 4.332" << endl;
        if (pmcInit->getFixed(extnptr->mapName)) {
          cerr << "check 4.34" << endl;
          fieldHasFixed[field] = true;
        }
        else {
          cerr << "check 4.35" << endl;
